...
@app.route("/api/v1/ansible-job/job-list", methods=["POST"])
def get_job_list():
    logger.info("Enter")
    response = None
    try:
        ...
        if (page_index < 1) or (page_size < 1):
            raise InvalidDataException("Both the values of 'page_index' and 'page_size' must be greater than or equal to 1.")
...
@app.route("/api/v1/ansible-job/codebase")
def access_codebase():
    pass

@app.route("/api/v1/ansible-job/job", methods=["POST"])
def create_job():
    logger.info("Enter")
    response = None
    try:
        ...
        if (not playbook_path) or (yml_index == -1):
            raise InvalidDataException("The job's 'playbook' must not be empty and must be a valid YAML file path like '<your_path>/test.yml'.")
            ...
            if not isinstance(credentials, dict):
                ...
                if username is not None:
                    ...
                    if (password is None) and (private_key_file is None):
                        raise MissingKeyException("Please provide either 'password' or 'private_key_file' corresponding to the 'username' : %s" % username)
                    else:
                    ...
                    if (password is not None) or (private_key_file is not None):
                        raise MissingKeyException("please provide 'username' corresponding to the 'password' or 'private_key_file'")
                ...
                if become_user is not None:
                    ...
                    if become_password is None:
                        raise MissingKeyException("Please provide the 'become_password' corresponding to the 'become_user' : %s" % become_user)
                    else:
                    ...
                    if become_password is not None:
                        raise MissingKeyException("Please provide the 'become_user' corresponding to the 'become_password'.")
...
@app.route("/api/v1/ansible-job/job-status/<job_id>")
...
@app.route("/api/v1/ansible-job/job/<job_id>")
...
@app.route("/api/v1/ansible-job/log/total-counts-grouped-by-categories/<job_id>")
...
@app.route("/api/v1/ansible-job/log/task-duration-grouped-by-names/<job_id>", methods=["POST"])
